...
export class WalletSyncService {
  ...
  private constructor() {
    ...
    this.blockfrostBaseUrl = apiConfig.blockfrostUrl || 'https://cardano-mainnet.blockfrost.io/api/v0';
  ...
  private fetchWalletBalance = async (address: string): Promise<BlockfrostResponse<WalletBalance>> => {
    try {
      ...
      const balance: WalletBalance = {
        lovelace: data.amount.find((a: any) => a.unit === 'lovelace')?.quantity || '0',
        assets: data.amount
  ...
  private fetchWalletUTXOs = async (address: string): Promise<BlockfrostResponse<CardanoUTXO[]>> => {
    try {
      ...
      const utxos: CardanoUTXO[] = data.map((utxo: any) => ({
        txHash: utxo.tx_hash,
