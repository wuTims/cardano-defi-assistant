...
export type SupabaseJWTPayload = {
  ...
  sub: string; // User UUID from app_users.id
  aud: string; // Always "authenticated"
  role: string; // Always "authenticated" 
  addr: string; // Wallet address for RLS policies
  wallet_type?: string; // Optional wallet type
...
// Supabase-specific token with required userId
export type SupabaseAuthToken = {
  token: string; // Supabase-verifiable JWT
  expiresAt: Date;
  walletAddress: string;
  walletType: WalletType;
  userId: string; // Required UUID from app_users table
};
