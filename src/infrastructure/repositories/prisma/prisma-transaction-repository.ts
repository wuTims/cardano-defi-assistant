...
import type { ITransactionRepository } from '@/core/interfaces/repositories';
import type { BulkInsertResult, CreateAssetFlowData, TransactionQueryFilters } from '@/core/types/database';
import type { Transaction, AssetFlow, Token } from '@prisma/client';
import { logger as rootLogger } from '@/lib/logger';

const logger = rootLogger.child({ repository: 'PrismaTransactionRepository' });

// Type alias for readability - matches interface return type
type TransactionWithAssets = Transaction & {
...
export class PrismaTransactionRepository implements ITransactionRepository {
  ...
  async saveBatch(
    ...
    assetFlows: Array<{ txHash: string; flows: CreateAssetFlowData[] }>,
    userId: string
   ...
   */
  async count(userId: string, filters?: TransactionQueryFilters): Promise<number> {
    try {
      const where: Prisma.TransactionWhereInput = {
        userId,
        ...(filters?.action && { txAction: filters.action }),
